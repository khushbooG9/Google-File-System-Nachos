...
//Create 4 chunk servers
Chunkserver c1("Node1"), c2("Node2"), c3("Node3"), c4("Node4");

//Creating the master server
void callmaster(int rep) {

    if(kernel->hostName == 100){
        Master *master = new Master(rep);
    master->add(&c1);
    master->add(&c2);
    master->add(&c3);
    master->add(&c4);
    cout<<"The Master server is created"<<endl;
    master->parse_commands();
    }
}

//Creating the chunk server
void callchunk(){
    //read *t = new Thread("Chunkserver");
    if (kernel->hostName == 201){
        cout<<"Chunkserver 1 is running"<<" "<<endl;
        c1();
    }
    else if(kernel->hostName == 202){
        cout<<"Chunkserver 2 is running"<<" "<<endl;
        c2();
    }
    else if(kernel->hostName ==203){
        cout<<"Chunkserver 3 is running"<<" "<<endl;
        c3();
    }
    else if(kernel->hostName == 204){
        cout<<"Chunkserver 4 is running"<<" "<<endl;
        c4();
    }
}
...
main(int argc, char **argv)
{
    ...
    else if(strcmp(argv[i], "-chunkserver")==0){
        ChunkserverFlag = TRUE;
    }
    ...
    if(MasterFlag){
        Thread *t = new Thread("Master Thread");
        t->Fork((VoidFunctionPtr) callmaster,(void *) 1);
    }
    if(ChunkserverFlag){
        Thread *t = new Thread("Chunkserver Thread");
        t->Fork((VoidFunctionPtr) callchunk, (void *) 1);
    }
