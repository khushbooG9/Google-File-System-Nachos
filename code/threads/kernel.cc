...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
	    ...
	    i++;
            ...
            //Execute this machine as a client
            callclient();
        }
...
#endif
            cout << "Partial usage: nachos [-n #] [-host #]\n";
	}
    }
}

void Kernel::callclient(){
    if(hostName==0) {

        //Send the messages to the master
        int farHost = 100;
        ...
        cout << "Got: " << buffer << " : from machine " << inPktHdr.from << ", box "
             << inMailHdr.from << "\n";
        cout.flush();
    } }
    else if(hostName==100)
    {
        cout<<"This is the master machine and not client";
        Exit(0);
    }
    else if (hostName>200){
        cout<<"This is supposed to be a chunk server.";
        Exit(0);
    }
    else{
        cout<<"Invalid client hostname";
        Exit(0);
    }
}
